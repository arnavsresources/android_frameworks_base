...
constexpr const char* kAwakenPath = "/system/framework/org.awakenos.platform-res.apk";
...
idmap2::Result<Idmap2Service::TargetResourceContainerPtr> Idmap2Service::GetTargetContainer(
  ...
  if (target_path == kAwakenPath) {
    if (awaken_apk_cache_ == nullptr) {
      // Initialize the awaken APK cache.
      auto target = TargetResourceContainer::FromPath(target_path);
      if (!target) {
        return target.GetError();
      }
      awaken_apk_cache_ = std::move(*target);
    }
    return {awaken_apk_cache_.get()};
  }
